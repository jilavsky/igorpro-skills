...
## 12. Common SAXS Patterns

### Process data in Python, write results back to Igor
```python
import igorpro, numpy as np
from scipy.optimize import curve_fit

df = igorpro.folder('root:SAS:sample1')
q  = igorpro.wave('q_DSM',  df).asarray()
I  = igorpro.wave('I_DSM',  df).asarray()
dI = igorpro.wave('dI_DSM', df).asarray()

def guinier(q, Rg, I0):
    return I0 * np.exp(-q**2 * Rg**2 / 3)

mask = q < 0.05
popt, _ = curve_fit(guinier, q[mask], I[mask],
                    sigma=dI[mask], absolute_sigma=True, p0=[10.0, I[0]])
Rg, I0 = popt

igorpro.variable.create(df.path() + 'Rg_py', Rg, overwrite=True)
igorpro.variable.create(df.path() + 'I0_py', I0, overwrite=True)
igorpro.print(f"Rg = {Rg:.2f} A,  I0 = {I0:.4g}")
```

### Batch process all sample subfolders
```python
import igorpro, numpy as np

for sample in igorpro.folder('root:SAS').subfolders():
    waves = {w.name(): w for w in sample.waves()}
    if 'q_DSM' not in waves or 'I_DSM' not in waves:
        continue
    q = waves['q_DSM'].asarray()
    I = waves['I_DSM'].asarray()
    invariant = np.trapz(I * q**2, q)
    igorpro.variable.create(sample.path() + 'invariant_py',
                            invariant, overwrite=True)
    igorpro.print(f"{sample.name()}: Q = {invariant:.4g}")
```

### Trigger Igor display update from Python
```python
igorpro.fn.IN4_DisplayCorrectControls()   # call user function
igorpro.execute("DoUpdate")               # run operation
```

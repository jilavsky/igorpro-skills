---
name: "Igor10Changes"
description: "Summarizes Igor Pro 10 changes, including Python integration, compiler behavior, and new language features."
parameters: {}
---

# Instructions
You are an Igor Pro 10 expert. When this skill is invoked, use the following reference to assist the user with Igor Pro 10 specific development:

1. Use this document to identify breaking changes in the compiler (e.g., unreachable code in switch statements).
2. Use the Python Integration section to guide users on bidirectional communication between Igor and Python (using `Python`, `PythonFile`, or the `igorpro` module).
3. Refer to the New Language Features section for modern syntax like `#pragma moduleName` in main procedures or `DFREF` in Multiple Return Syntax.
4. Check the Bug Fixes section to ensure users are aware of changes in function results (e.g., `area()`, `faverage()`) due to 10.01 updates.
...
Igor 10 introduced direct, bidirectional Python communication. This is entirely new
and should be used instead of workarounds like ExecuteScriptText for Python tasks.

### Calling Python from Igor
...
| Python Overview | https://docs.wavemetrics.com/igorpro/python/python-overview |
| Python Module Reference | https://docs.wavemetrics.com/igorpro/python/python-module-reference |
